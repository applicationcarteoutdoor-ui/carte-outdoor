...
Carte Outdoor : PWA statique de cartographie de points d'intérêt outdoor (via ferrata, refuges, escalade, châteaux, cités de caractère, GR). **Vanilla JS + modules ES, aucun build, aucune dépendance npm** — les librairies (Leaflet, markercluster, togeojson, fflate) sont vendorées dans `vendor/`. Interface, commentaires et documentation en **français**. Pas de dépôt git. ~7 400 points par défaut + 190 tracés GR + ~66 500 toilettes (fichier séparé `data/toilettes.geojson`, chargé à la demande, non pré-caché).
...
- **`js/storage.js`** — toute la persistance, API volontairement async pour pouvoir brancher un back-end plus tard sans toucher au reste. localStorage (prefs, statuts, overrides, customThemes, idées) ; IndexedDB v2 (points utilisateur, traces, carnet avec photos).
